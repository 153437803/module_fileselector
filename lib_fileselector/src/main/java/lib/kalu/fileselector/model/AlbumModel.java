...
import lib.kalu.fileselector.cursorloader.AlbumTreeCursorLoader;
...
public class AlbumModel implements Serializable {
    ...
    public static AlbumModel valueOf(Cursor cursor) {
        String clumn = cursor.getString(cursor.getColumnIndex(AlbumTreeCursorLoader.COLUMN_URI));
        return new AlbumModel(
                cursor.getString(cursor.getColumnIndex("bucket_id")),
                Uri.parse(clumn != null ? clumn : ""),
                cursor.getString(cursor.getColumnIndex("bucket_display_name")),
                cursor.getLong(cursor.getColumnIndex(AlbumTreeCursorLoader.COLUMN_COUNT)));
    }
